...
import { ComisionAsignarTutorComponent } from './asignar-tutor';

describe('ComisionAsignarTutorComponent', () => {
  let component: ComisionAsignarTutorComponent;
  let fixture: ComponentFixture<ComisionAsignarTutorComponent>;
  let httpMock: HttpTestingController;

  function createComponent() {
    fixture = TestBed.createComponent(ComisionAsignarTutorComponent);
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [ComisionAsignarTutorComponent, HttpClientTestingModule],
    }).compileComponents();
  ...
  it('should create and load initial data', () => {
    ...
    httpMock.expectOne('/api/uic/admin/estudiantes-sin-tutor?careerId=null&academicPeriodId=null').flush([]);
    httpMock.expectOne('/api/uic/admin/asignaciones/tutor?careerId=null&academicPeriodId=null').flush([]);
  ...
  it('should refresh lists onChangeCarrera/onChangePeriodo', () => {
    ...
    httpMock.expectOne('/api/uic/admin/estudiantes-sin-tutor?careerId=10&academicPeriodId=2').flush([]);
    httpMock.expectOne('/api/uic/admin/asignaciones/tutor?careerId=10&academicPeriodId=2').flush([]);
  ...
  it('asignar should validate ids', () => {
    ...
    component.tutorId = undefined as any;
    component.asignar();
  ...
  it('asignar should call backend and refresh on success', () => {
    ...
    component.tutorId = 20;
    component.asignar();

    const req = httpMock.expectOne('/api/uic/admin/asignaciones/tutor');
    expect(req.request.method).toBe('PUT');
    expect(req.request.body).toEqual({ id_user_student: 10, tutor_usuario_id: 20, academicPeriodId: 4 });
    req.flush({});

    httpMock.expectOne('/api/uic/admin/estudiantes-sin-tutor?careerId=null&academicPeriodId=4').flush([]);
    httpMock.expectOne('/api/uic/admin/asignaciones/tutor?careerId=null&academicPeriodId=4').flush([]);

    expect(component.message).toBe('Tutor asignado correctamente');
  });
  ...
  it('asignar should set error on backend error', () => {
    ...
    // ensure no pending refresh requests (component may still attempt to refresh on complete)
    httpMock.match((r) => r.url.startsWith('/api/uic/admin/estudiantes-sin-tutor')).forEach((r) => r.flush([]));
    httpMock.match((r) => r.url.startsWith('/api/uic/admin/asignaciones/tutor')).forEach((r) => r.flush([]));
  ...
  it('cargarPeriodoActivo should auto-select active period when none selected', () => {
    ...
    httpMock.expectOne('/api/uic/admin/estudiantes-sin-tutor?careerId=null&academicPeriodId=9').flush([]);
    httpMock.expectOne('/api/uic/admin/asignaciones/tutor?careerId=null&academicPeriodId=9').flush([]);
