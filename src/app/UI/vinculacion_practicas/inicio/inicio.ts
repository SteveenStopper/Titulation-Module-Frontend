...
export class Inicio {
  ...
  constructor(private notificationsSvc: NotificationsService, private http: HttpClient) {
    ...
    this.http.get<any>('/api/vinculacion/dashboard').subscribe((d) => {
      this.kpis = {
        elegibles: Number(d?.elegibles || 0),
        vinculacionGuardadas: Number(d?.vinculacionGuardadas || 0),
        vinculacionValidadas: Number(d?.vinculacionValidadas || 0),
        practicasGuardadas: Number(d?.practicasGuardadas || 0),
        practicasValidadas: Number(d?.practicasValidadas || 0),
        certificadosEmitidosHoy: Number(d?.certificadosEmitidosHoy || 0),
      };
    });
    this.http.get<Array<{ estudiante: string; tramite: string; fecha: string; estado: 'completado'|'pendiente' }>>('/api/vinculacion/recientes')
      .subscribe((rows) => {
