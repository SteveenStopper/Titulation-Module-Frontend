...
describe('Inicio', () => {
  ...
  function flushInitHttpCalls() {
    httpMock.expectOne('/api/vinculacion/dashboard').flush({});
    httpMock.expectOne('/api/vinculacion/recientes').flush([]);
  }
  ...
  it('should map dashboard KPIs and recientes list (array)', () => {
    ...
    httpMock.expectOne('/api/vinculacion/dashboard').flush({
      elegibles: 1,
      vinculacionGuardadas: 2,
      vinculacionValidadas: 3,
      practicasGuardadas: 4,
      practicasValidadas: 5,
      certificadosEmitidosHoy: 6,
    });
    httpMock.expectOne('/api/vinculacion/recientes').flush([
      { estudiante: 'E', tramite: 'T', fecha: '2026-01-01T00:00:00.000Z', estado: 'pendiente' },
    ]);

    expect(component.kpis.elegibles).toBe(1);
    expect(component.kpis.certificadosEmitidosHoy).toBe(6);
