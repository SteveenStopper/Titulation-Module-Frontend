...
import { CalificacionPracticas } from './calificacion';
import { AuthService } from '../../../../services/auth.service';

describe('Calificacion', () => {
  let component: CalificacionPracticas;
  let fixture: ComponentFixture<CalificacionPracticas>;
  let httpMock: HttpTestingController;
  let authMock: { currentUserValue: any };

  function createWithUser(user: any) {
    authMock.currentUserValue = user;
    fixture = TestBed.createComponent(CalificacionPracticas);
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [CalificacionPracticas, HttpClientTestingModule],
      providers: [
        {
          provide: AuthService,
          useValue: authMock,
  ...
  it('should load eligible list when allowed (Administrador)', () => {
    ...
    httpMock.expectOne('/api/practicas/eligible').flush([
  ...
  it('should load eligible list when allowed (Vinculacion_Practicas)', () => {
    ...
    httpMock.expectOne('/api/practicas/eligible').flush([{ id_user: 10, fullname: 'E1', score: 80, status: null }]);
  ...
  it('guardar should post save-for and set guardado on success and alert on error', () => {
    ...
    const okReq = httpMock.expectOne('/api/practicas/save-for');
    ...
    component.guardar(10);
    const errReq = httpMock.expectOne('/api/practicas/save-for');
  ...
  it('generarCertificado should return early when item is not found', () => {
    ...
    httpMock.match('/api/practicas/certificate').forEach(r => r.flush(new Blob(['x'])));
    expect(component.items).toEqual([]);
  });
});
