...
export class CalificacionPracticas {
  ...
  guardar(id: number) {
    ...
    this.http.post('/api/practicas/save-for', { target_user_id: id, score }).subscribe({
  ...
  generarCertificado(id: number) {
    const it = this.items.find(x => x.id === id);
    if (!it) return;
    this.http.post('/api/practicas/certificate', {}, { responseType: 'blob', observe: 'response' }).subscribe({
      next: (resp) => {
  ...
  private cargarElegibles() {
    this.http.get<Array<{ id_user: number; fullname: string; score?: number|null; status?: string|null }>>('/api/practicas/eligible')
      .subscribe(rows => {
